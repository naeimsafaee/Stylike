...
exports.getSystemWalletsReal = async (req, res) => {
	try {
		const data = await walletServices.getSystemWalletsReal(req.query);
		return response({ res, statusCode: httpStatus.OK, data });
	} catch (e) {
		if (!e.statusCode) e = { statusCode: 500, status: "Internal Error", message: e.message };
		return res.status(e.statusCode).json(e);
...
exports.createEth = async (req, res) => {
	try {
		const data = await walletServices.createEth(req.body);
		return response({ res, statusCode: httpStatus.OK, data });
	} catch (e) {
		if (!e.statusCode) e = { statusCode: 500, status: "Internal Error", message: e.message };
		return res.status(e.statusCode).json(e);
...
exports.updateWallets = async (req, res) => {
	try {
		const data = await walletServices.updateWallets(req.body);
		return response({ res, statusCode: httpStatus.OK, data });
	} catch (e) {
		if (!e.statusCode) e = { statusCode: 500, status: "Internal Error", message: e.message };
		return res.status(e.statusCode).json(e);
