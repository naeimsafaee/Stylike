...
const {UserActivity} = require("../../databases/postgres");
...
async function getPriceHistory(data) {
    ...
    const result = await UserActivity.findAll(query);
    ...
    result.forEach((p) => {
        ...
        if (foundedPriceIndex !== -1) {
            filtered[foundedPriceIndex].price = add(filtered[foundedPriceIndex].price, p.price);
            filtered[foundedPriceIndex].count++;
            filtered[foundedPriceIndex].avg = div(
