...
const {HumanError, NotAuthorizedError} = require("./errorhandler");
...
async function addToken(
    ...
    if (collectionId) {
        const thisCollcetion = await UserCollection.findOne({where: {id: collectionId, userId: user.id}});
        if (!thisCollcetion) {
            throw (
                new HumanError(Errors.USER_COLLECTION_NOT_FOUND.MESSAGE, Errors.USER_COLLECTION_NOT_FOUND.CODE)
            );
        }
    }
    ...
    if (fileValidationError) {
        throw (
            new HumanError(Errors.FILE_NOT_SUPPORTED.MESSAGE, Errors.FILE_NOT_SUPPORTED.CODE, {
                fileValidationError,
            })
        );
    }
    ...
    if (properties) {
        if (!isJson(properties)) {
            throw (new HumanError(Errors.INVALID_PROPERTIES.MESSAGE, Errors.INVALID_PROPERTIES.CODE));
        }
    }

    if (!files.nftFile) {
        throw (new HumanError(Errors.NFT_FILE_UPLOAD_ERROR.MESSAGE, Errors.NFT_FILE_UPLOAD_ERROR.CODE));
    }
    ...
    const thisFee = await Settings.findOne({where: {key: "MINT_FEE_" + chain}, raw: true})
    const thisFeeAmount = thisFee ? parseFloat(thisFee.value) : 0;
    ...
    const result = await UserToken.create({
        ...
        serialId,
        fee: thisFeeAmount,
        properties: properties ? formatedProperties : [],
    ...
    if (!result) throw(new HumanError(Errors.USER_TOKEN_FAILED.MESSAGE, Errors.USER_TOKEN_FAILED.CODE));
...
function updateToken(userId, tokenId, txId) {
    return new Promise(async (resolve, reject) => {
        ...
        let token = await UserToken.findOneAndUpdate({id: tokenId}, {deletedAt: new Date()});

        if (!token)
            return reject(
                new HumanError(Errors.USER_TOKEN_NOT_FOUND.MESSAGE, Errors.USER_TOKEN_NOT_FOUND.CODE, {
                    id: tokenId,
                }),
            );
...
async function getToken(id, user) {
    ...
    if (!result)
        throw (
            new HumanError(Errors.USER_TOKEN_NOT_FOUND.MESSAGE, 400)
        );
    ...
    //todo
    // if (user) {
    //     let like = await UserFollowLike.findOne({userId: user.id, likedToken: new ObjectId(id)});
    ...
    const currentAssignedToken = await UserAssignedToken.findOne({
        where: {
            status: ["FREE", "IN_AUCTION", "PENDING", "NOT_MINTED"],
            deletedAt: null,
            tokenId: id,
        },
    ...
    if (!currentAssignedToken) {
        throw (
            new HumanError(Errors.USER_TOKEN_NOT_FOUND.MESSAGE, 404)
        );
    }

    if (String(currentAssignedToken.userId.id) != String(user?.id) && currentAssignedToken.status == "PENDING") {
        throw (
            new HumanError(Errors.USER_TOKEN_NOT_FOUND.MESSAGE, 404)
        );
    }
    ...
    activeAuction = await UserAuctions.findOne({
        where: {
            assignTokenId: currentAssignedToken.id,
            status: "ACTIVE",
            // start: { [Op.lt]: currentDate },
    ...
    const offerQuery = {
        where: {
            ...
            status: "REGISTER",
            [postgres.Op.or]: [{assignedTokenId: currentAssignedToken.id}],
        },
    };

    if (activeAuction) {
        offerQuery.where[postgres.Op.or].push({auctionId: activeAuction.id});
    }
    const thisTempOffers = await UserAuctionsOffer.findAll({
        where: offerQuery.where,
        include: [
            {model: User, attributes: ["avatar", "username", "email", "id", "address"]},
        ],
        order: [["amount", "DESC"]],
        raw: true,
    });
    ...
    //     id: {$ne: currentAssignedToken.id},
    // })
    //     .limit(10)
    //     .populate("userId", "id image email username address")
    //     .populate("tokenId")
    //     .populate("collectionId", "name description image")
    //     .lean();

    const similiars = await UserAssignedToken.findAll({
        where: {
            collectionId: currentAssignedToken.collectionId,
            status: ["FREE", "IN_AUCTION"],
            id: {[postgres.Op.ne]: currentAssignedToken.id},
        },
        limit: 10,
        include: [
            {model: User, attributes: ["id", "avatar", "email", "username", "address" , 'name']},
            {model: UserToken},
            {model: UserCollection, attributes: ["name", "description", "image"]},
        ],
        raw: true,
    });
    ...
    for (let i = 0; i < similiars.length; i++) {
        ...
        //         where: {
        //             userId: user.id,
        //             likedToken: similiars[i].tokenId.id,
        //         },
        //     });
        //     if (like) {
        //         is_liked = true;
    ...
    const tokenIds = similiars.map((sm) => String(sm.tokenId.id));
    const filtered = similiars.filter(({ tokenId }, index) => !tokenIds.includes(String(tokenId.id), index + 1));


    // const similiars = await UserToken.find({ collectionId: result.collectionId, id: { $ne: id } }).lean();
    return {
        token: { ...result },
...
function getTokenByManager(id) {
    return new Promise(async (resolve, reject) => {
        const result = await UserToken.findOne({id: id})
            .populate({
                path: "collectionId",
                match: {deletedAt: null},
                populate: {
...
function editUserTokenByManager(id, isTrend, isSlider) {
    return new Promise(async (resolve, reject) => {
        const thisToken = await UserToken.findOne({id: new ObjectId(id), deletedAt: null});
...
function getTokensCount(user) {
    const count = UserAssignedToken.count({
        where: {
            userId: user.id,
            deletedAt: null,
            status: ['FREE', 'IN_AUCTION', 'NOT_MINTED'],
        },
    });
...
function importToken(user, {chain, address}) {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            return reject(new HumanError(Errors.ADD_FAILED.MESSAGE, Errors.ADD_FAILED.CODE));
        }
    });
}
