...
// const {populate} = require("../../databases/postgres/models/CollectionEngagement");
...
async function getUser(id, requsetedUserId) {
    let user;
    let result = await User.findOne({where: { id: id }});

    if (!result)
        throw new HumanError(Errors.USER_NOT_FOUND.MESSAGE, 404);

    const collectionsCount = await UserCollection.count({where: { userId: id }});

    if (requsetedUserId) {
        const getRequsetedUser = await User.findOne({where: { id: requsetedUserId }});
        user = await serializeUser(result, getRequsetedUser);
        return user;
    }

    const nftsCount = await UserAssignedToken.count({
        where: {
            userId: id,
...
async function referralHistory({ page, limit, order, type }, user) {
    ...
    let result = await ReferralReward.aggregate([
        ...
        {
            $group: {
                _id: { referredUserId: "$referredUserId", assetId: "$assetId" },
